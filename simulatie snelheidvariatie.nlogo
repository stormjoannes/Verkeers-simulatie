...
to create-highway
  ...
  if pxcor < 2 and pxcor > 0 and pycor < 6 and pycor > 2 [ set pcolor gray ] ;;set stripe way 1
  if pxcor < 2 and pxcor > 0 and pycor < -2 and pycor > -6 [ set pcolor gray ] ;; set stripe way 2
end
...
to make-cars [ amount road-num]
  create-cars amount [
    ...
    set speed 0
    ifelse road-num = 1 [ set speed-limit 0.5 ] [ set speed-limit 1 ];; door de waarde bij de auto te zetten krijgt de weg een maximum snelheid
    seperate
  ]
end
...
to jam_way_1
  ; verlaag de snelheid van 1 auto op baan 1 om een file te veroorzaken
...
to jam_way_2
 ; verlaag de snelheid van 1 auto op baan 2 om een file te veroorzaken
...
to slow-down [car-infront]
  ...
  if chance < 90 [
    ; versloom een auto op basis van de auto voor hem en de decelaration
    set speed [speed] of car-infront - decelaration
  ]
end

to speed-up []
  ; versnel een auto op basis van de speed en de acceleration
